<?php
  ...
  {
    $myPage = new page();
    $myPage->set_title("Manager > Spiele");
    ...
    $myQuery->set_sql_select("SELECT *, p1.user_account as p1_fullname, 
                                        p2.user_account as p2_fullname, 
                                        p3.user_account as p3_fullname, 
                                        p4.user_account as p4_fullname, 
                                        winner.user_account as winner_fullname, 
                                        winner2.user_account as winner2_fullname, 
                                        DATE_FORMAT(group_created,'%d.%m.%Y') as group_created_c
        											FROM games
    													...
    													LEFT JOIN users as winner2 ON game_winner2_id = winner2.user_id
    													");
    ...
    $db->sql_query("SELECT *, user_account as winner2_fullname FROM users ORDER BY user_account");
    $myCol = new column("game_winner2_id","Gewinner 2"); $myCol->set_selection_by_sql($db,'winner2_fullname','user_id'); $myCol->set_filter_column('winner2.user_account'); $myQuery->add_column($myCol);
