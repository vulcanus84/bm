...
void readSensor() {
  ...
  if (found) {
    ...
    if (zone != lastZone && zone != "") {
      ...
      Serial.println(zone);
      evaluateZone(zone);  // deine eigene Auswertung

      // Optional: LED-Feedback an Matrix senden
      /*
      LedCommand cmd;
      cmd.type = SHOW1; // oder je nach Zone
...
void taskRadar(void *pvParameters) {
  ...
  for (;;) {
    readSensor();
    vTaskDelay(pdMS_TO_TICKS(50)); // 50ms Zyklus, ~20Hz
  }
}
