...
void taskGameControl(void *pvParameters) {
  for (;;) {
    if (millis() - lastConfigSend > 2000) {
      String config = getConfigFromServer(configParams);
      if (config != "") {
        readConfig(config);
      }
      lastConfigSend = millis();
    }
    vTaskDelay(10 / portTICK_PERIOD_MS); // Kurze Verzögerung für Stabilität
  }
}
...
void readConfig(String payload) {
  ...
  if (!err) {
    ...
    configParams = "?status=" + gameStatus +
...
void sendConfig() {
  String configString = "?status=" + gameStatus +
          "&user_id=" + String(userId) +
          "&sequence=" + getSequenceAsString() +
          "&nextPos=" + getNextSequenceId();
}
